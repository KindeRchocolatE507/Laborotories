...
#include "data.h"
#include "individual_function/scholarship_female_students.h"
...
int main(int argc, char *argv[])
{
    if (argc != 1)
    {
        ...
        tmp = new = (select_type *)malloc(size);
        ...
        root_tmp = new;                             //new - последний элемент
        printf("%s\n", "Welcome!");
        if (root_tmp)
        {
            ...
            {
                scholarship_female_students(tmp);
                getchar();
                printf("%s", "Goodbye!\n");
                return 0;
            }
